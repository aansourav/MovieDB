...
const getMovieById = (id) => {
    return (
        movieList.results.find((movie) => movie.id === id) || {
            message: "Movie not found",
        }
    );
};

const patchMovie = (id, title) => {
    const movie = movieList.results.find((movie) => movie.id === id);
...
const deleteMovie = (id) => {
    ...
    const movie = movieList.results[movieIndex];
    movieList.results.splice(movieIndex, 1);
    return movie;
};

export { deleteMovie, getAllMovies, getMovieById, patchMovie };
